...
-- ──────────────────────────────────────────
-- 0. 기존 정책 삭제 (재실행 시 중복 오류 방지)
-- ──────────────────────────────────────────
DROP POLICY IF EXISTS "allow_all_sd_users"    ON sd_users;
DROP POLICY IF EXISTS "allow_all_sd_tasks"    ON sd_tasks;
DROP POLICY IF EXISTS "allow_all_sd_reports"  ON sd_reports;
DROP POLICY IF EXISTS "allow_all_sd_settings" ON sd_settings;
DROP POLICY IF EXISTS "allow_all_sd_notif"    ON sd_notif_log;
...
--    (앱 레벨에서 자체 인증 처리)
-- ──────────────────────────────────────────
CREATE POLICY "allow_all_sd_users"
  ON sd_users FOR ALL TO anon
  USING (true) WITH CHECK (true);

CREATE POLICY "allow_all_sd_tasks"
  ON sd_tasks FOR ALL TO anon
  USING (true) WITH CHECK (true);

CREATE POLICY "allow_all_sd_reports"
  ON sd_reports FOR ALL TO anon
  USING (true) WITH CHECK (true);

CREATE POLICY "allow_all_sd_settings"
  ON sd_settings FOR ALL TO anon
  USING (true) WITH CHECK (true);

CREATE POLICY "allow_all_sd_notif"
  ON sd_notif_log FOR ALL TO anon
  USING (true) WITH CHECK (true);
